...
public class KafkaConsumerConfig {
  ...
  public DefaultKafkaConsumerFactory<String, ChatMessageDto.Response> consumerFactory() {
    ...
    // 카프카브로커 설정 여러개 설정하는것 권장(현재는 하나만 설정)
    consumerProps.put(ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG, bootstrapAddress);
    // 그룹 아이디는 컨슈머 그룹이라고도 함(다른 컨슈머 그룹에 속한 컨슈머들은 서로 영향 X)
    consumerProps.put(ConsumerConfig.GROUP_ID_CONFIG, "chat-group");

    // 이전 메세지를 필요로 하거나 중복 메시지를 감수할 수 있다면 earliest
    // 현재 이후의 메시지만 필요하다면 latest
    consumerProps.put(ConsumerConfig.AUTO_OFFSET_RESET_CONFIG, "latest");
    // 데이터베이스에 메세지를 저장 후 커밋을 하기 위함
  ...
  public ConcurrentKafkaListenerContainerFactory<String, ChatMessageDto.Response> chatKafkaListenerContainerFactory() {
    ...
    // ack.acknowledge(); 까지 돼야 다음 데이터를 처리 (수신에 성공했다고 명시적으로 호출해줘야함)
