...
* */
  ...
  public static PageRequest of(int page, PostOrder postOrder) {
    page = page <= 0 ? 1 : page;
    return PageRequest.of(page-1, size, Sort.by(Sort.Direction.DESC, postOrder.getOrderTarget()));
    // 페이지가 1부터 시작하는 것을 조정 (1페이지를 보고싶을땐 실제로 0페이지에 해당하는 데이터 넘겨야함
  }

  public static PageRequest createPageRequestByPostOrder(int page, PostOrder postOrder) {
    page = page <= 0 ? 1 : page;
