...
public class KafkaChatService {
  ...
  private final KafkaTemplate<String, ChatMessageDto.Response> kafkaChatMessageTemplate;   //kafkaChatMessageTemplate
  private final String CHAT_TOPIC = "chat3";



  public void publishChatMessage(ChatMessageDto.Response chatMessage) { // 특정 토픽에 메세지를 보낸다.

    kafkaChatMessageTemplate.send(CHAT_TOPIC, chatMessage); // 파티션은 생략했다.
...
//}
  ...
  @KafkaListener(topics = CHAT_TOPIC, groupId = "chat-group", containerFactory = "chatKafkaListenerContainerFactory")   // 카프카의 특정 토픽에 온 이벤트를 처리
  public void consumeAndBroadcast(ChatMessageDto.Response chatMessage) {
    ...
    simpMessagingTemplate.convertAndSend("/sub/chat/" + postId, chatMessage);
